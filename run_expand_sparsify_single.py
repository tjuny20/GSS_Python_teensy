#!/usr/bin/env python3
"""
Expand-and-sparsify gridsearch for GPU server (single-file version).

Runs the full p_hd x d sweep for a single data file, splitting labeled data
into train/test via stratified random splits. Computes cosine similarities
(input vs output inner products) and linear SVM accuracy on z_hd. Saves
results as pickles that the Jupyter notebooks can load directly.

Usage:
    python run_expand_sparsify_single.py
"""
