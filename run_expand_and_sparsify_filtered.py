...
#!/usr/bin/env python3
"""
Expand-and-sparsify gridsearch for GPU server — filtered configs.

Configs (no 32-trace, no 176-trace):
    raw                     (8 traces)
    ∂¹                      (16 traces)
    8R                      (16 traces)
    8D                      (16 traces)
...
Usage:
    python run_expand_and_sparsify_filtered.py
"""
...
# Filtered table: 8, 16, 24, 40, 136 traces (no 32, no 176)
CONFIG_SPECS = {
    # 8 traces
    'raw':                  (0, 0, 0),
    # 16 traces
...
def run_gridsearch_single(filename, pkl_path, n_train=450,
    ...
    # ---- pre-sample pairs for all repeats (shared across configs) ----
    all_seed_pairs = {}
    for seed in range(n_repeats):
        all_seed_pairs[seed] = sample_pairs(n_sensors, seed)
    ...
    for ci, cfg_name in enumerate(config_names):
        ...
        # -- resolve pairs per seed from the shared pool --
        seed_pairs = {}
        pairs_list = []
        for seed in range(n_repeats):
            if resample:
                rp, dp = all_seed_pairs[seed]
...
if __name__ == '__main__':
    ...
    run_gridsearch_single(
        '1_600_20',
        'data/expand_sparsify_filtered_results.pkl',
        n_train=450)

    # Binary mixtures: separate train/test files
    run_gridsearch_binary(
        'mix_100_20_1', 'mix_50_20_1',
        'data/expand_sparsify_filtered_mix_results.pkl')
